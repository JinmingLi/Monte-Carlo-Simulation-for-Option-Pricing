...
int main()
{
	...
	{ // Calculate a path at each iteration
		...
		for (long index = x.MinIndex()+1; index <= x.MaxIndex(); ++index)
		{
			...
			VNew = VOld  + (k * SDEDefinition::drift(x[index-1], VOld))
						+ (sqrk * SDEDefinition::diffusion(x[index-1], VOld) * dW);
	...
	// standard deviation
	double sd = sqrt((c1 - c2) / (NSim - 1)) * exp(-myOption.r * myOption.T);
	// standard error
	double se = sd / sqrt(double(NSim));
